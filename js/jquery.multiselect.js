    ...
    $.fn.multiselect = function( options ){
        ...
        return this.each(function(){
            ...
            if( options.selectAll ) {
                optionsList.before('<input type="checkbox" id="' + options.checkboxSelectAllId  + '" class="' + options.checkboxSelectAllClass + '" /><a href="#" class="ms-selectall global">'+ options.selectAllText +'</a>');

                var globalSelectAll = optionsWrap.find('.ms-selectall.global');
            }

			if( options.customButtons ) {
				$( '.' + options.checkboxSelectAllClass ).after( options.customButtons );
...
//                    });
					...
					checkbox_id = $(this).attr('label');
					if (typeof checkbox_id !== typeof undefined && checkbox_id !== false) {
						checkbox_id = checkbox_id.replace(/\s+/g, '-').toLowerCase();
						checkbox_label = $(this).attr('label');
					} else {
						checkbox_label = 'Select All';
					}

                    if ( options.selectGroup && checkbox_label.length > 0 ) {
                        container.append('<input type="checkbox" id="'+checkbox_id+'" class="ms-selectall-checkbox" /><a href="#" class="'+ checkbox_id +' ms-selectall">' + checkbox_label + '</a>');
                    }
